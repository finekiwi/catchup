...
def _make_text_item(text: str, label_value: str = "text", page_no: int = 1) -> MagicMock:
...
def test_parse_pdf_marks_failure_when_docling_unavailable(monkeypatch, tmp_path: Path) -> None:
...
def test_parse_pdf_unknown_label_defaults_to_text(tmp_path: Path) -> None:
    ...
    item = _make_text_item("Unknown typed content", label_value="unknown_type_xyz", page_no=1)
    mock_result = _make_conversion_result([item])
