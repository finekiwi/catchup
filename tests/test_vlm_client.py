"""Tests for vlm/client.py wrappers — all API calls are mocked."""
...
from utils.models import MODEL_REGISTRY, compute_cost  # noqa: E402
from vlm.client import call_vlm  # noqa: E402
...
def test_call_vlm_openai_failure(fake_image: Path):
    with patch("openai.OpenAI") as mock_cls, patch("vlm.client.log_api_call"):
        mock_client = MagicMock()
        mock_cls.return_value = mock_client
...
def test_supported_models_contains_all_providers():
    actual_providers = {v["provider"] for v in MODEL_REGISTRY.values()}
