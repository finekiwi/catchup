...
def _notebook(
    cells: list[dict[str, object]], metadata: dict[str, object] | None = None
) -> SimpleNamespace:
...
def _code_cell(
    source: str, outputs: list[dict[str, object]] | None = None
) -> dict[str, object]:
...
def test_parse_ipynb_basic_cells(mock_read, tmp_path: Path) -> None:
    ...
    notebook = _notebook(
        cells=[_markdown_cell("## Intro"), _code_cell("print('hello')")]
    )
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_image_output_creates_figure_block(
    mock_read, tmp_path: Path
) -> None:
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_broken_file_returns_fallback_document(
    mock_read, tmp_path: Path
) -> None:
...
def test_parse_ipynb_metadata_cell_index_and_type(mock_read, tmp_path: Path) -> None:
    ...
    metadata_pairs = [
        (block.metadata.cell_index, block.metadata.cell_type)
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_error_output_traceback_to_text_block(
    mock_read, tmp_path: Path
) -> None:
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_error_output_without_traceback_uses_ename_evalue(
    mock_read, tmp_path: Path
) -> None:
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_error_output_without_details_uses_generic_text(
    mock_read, tmp_path: Path
) -> None:
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_unknown_output_type_is_ignored_and_order_is_continuous(
    mock_read, tmp_path: Path
) -> None:
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_uses_language_info_over_kernelspec(
    mock_read, tmp_path: Path
) -> None:
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_uses_kernelspec_language_when_language_info_missing(
    mock_read, tmp_path: Path
) -> None:
    """Uses kernelspec language when language_info metadata is absent."""
    notebook = _notebook(
        cells=[_code_cell("x <- 1")], metadata={"kernelspec": {"language": "r"}}
    )
    mock_read.return_value = notebook
    file_path = _touch_notebook_file(tmp_path)
...
@patch("parsers.ipynb_parser.nbformat.read")
def test_parse_ipynb_defaults_language_to_python_without_metadata(
    mock_read, tmp_path: Path
) -> None:
